...
//our code:
//the thunk contain the argument environment and the expression 
//the procedure application is being evaluat.
export type thunk ={tag:"thunk", arg: CExp4, env: Env}


// the type is predicates
export const isThunk = (x: any): x is thunk => x.tag==="thunk";


// the type is value constructors
export const makeThunkObject = (arg: CExp4, env: Env): thunk => ({tag:"thunk", arg: arg, env: env});


// force or delay arguments
export const force = (x: thunk): Value4 | Error => L4applicativeEval(x.arg, x.env);

export const forceThunks=(args:(thunk | Value4)[])=> {
    let forced = map((arg: thunk | Value4) => isThunk(arg) ? force(arg) : arg ,args);
        return hasNoError(forced)? 
       
        forced :
        Error(`Error when we trying to force the thunks`);
}

export const evalEnv= (proc: Closure4, args: CExp4[], applicationEnv: Env): Array<Value4 | thunk> | Error => {
    let zipped = zip(proc.params, args);
    let evaled: Array<Value4 | thunk>  = map((pair): Value4 | thunk | Error =>

        first(pair).lazy? 
              makeThunkObject(second(pair), applicationEnv):
              L4applicativeEval(second(pair), applicationEnv)
    , zipped);

    return hasNoError(evaled) ?
    
        evaled :
        Error(`Error in :Normal eval`);
};
...
const newApplyClosure4 = (proc: Closure4, args: CExp4[],env:Env): Value4 | Error => {
    const evalArgs: (Value4|thunk)[] | Error = evalEnv(proc, args, env);
...
const evalLet4 = (exp: LetExp4, env: Env): Value4 | Error => {
    ...
    map((binding) => binding.var.lazy? 
        makeThunkObject(binding.val,env) : 
