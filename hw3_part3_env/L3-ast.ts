...
//function that check if the variable is lazy.
export const checkLazy=(sexps: string[] | string ): VarDecl | Error =>
((sexps.length===2) && (sexps[1]==="lazy") ? 
    makeVarDecl(sexps[0], true):

    (isString(sexps) ? 
        makeVarDecl(sexps, false):

        Error('The Expression is not string')
    )
 );
...
const parseProcExp = (sexps: any[]): ProcExp | Error =>
    safeFL((body: CExp[]) => makeProcExp( map(checkLazy, sexps[1]), body))
        (map(parseL3CExp, rest(rest(sexps))));
//our changes.
        const isStringArray=(y:any): y is  String[] => ( isArray(y) && allT(isString,y));

export const parseDecls = (sexps: any[]): VarDecl[] | Error =>{
    if(allT(isString, sexps)){
        let decls: (VarDecl|Error)[]= map(checkLazy, sexps);
