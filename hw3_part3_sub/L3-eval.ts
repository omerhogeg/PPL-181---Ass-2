...
const L3applyProcedure = (proc: Value | Error, args: CExp[], env: Env): Value | Error =>
    ...
    isPrimOp(proc) ? ourPrimOpHandle(proc,args,env) :
    isClosure(proc) ? ourClosureHandle(proc,args,env) :
    Error("Bad procedure " + JSON.stringify(proc))
//our code: 
//ourClosureHandle funtion.
const ourClosureHandle = (proc: Closure, args: CExp[], env: Env ): Value | Error =>{

    const zipped = zip(proc.params, args);
    const lazyCh = map((p)=> first(p).lazy? second(p): L3applicativeEval(second(p), env), zipped);
    const vars = map((p) => p.var, proc.params);
    const body = renameExps(proc.body);

    return hasNoError(lazyCh) ? evalExps(substitute(body, vars, map((arg)=> isCExp(arg)? arg : valueToLitExp(arg), lazyCh)), env):
        Error(getErrorMessages(lazyCh));
    };

//ourPrimOpHandle function.   
const ourPrimOpHandle=(proc:PrimOp,args:CExp[],env:Env): Value | Error =>{

    const values:Array <Value | Error>=map((arg) => L3applicativeEval(arg,env),args);

    if (hasNoError(values))
        return applyPrimitive(proc, values);

    else
        return Error(getErrorMessages(values));
    };
